...
| `--disable-browser-sandbox` | Off by default: Chromium runs with its own process sandbox intact. This tool's job is navigating to arbitrary, often-external URLs, so the sandbox is the primary containment boundary between a compromised renderer and this host. Pass this flag only when the deployment cannot run a sandboxed Chromium (e.g. some containers running as root without the SUID sandbox helper) and no other isolation (container/VM) wraps the scan. |
